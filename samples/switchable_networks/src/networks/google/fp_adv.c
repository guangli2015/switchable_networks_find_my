...
static const struct app_fp_adv_cb *registered_cb;
...
static void fp_adv_restart_work_handle(struct k_work *work);
...
static void fp_advertising_update(void)
{
	...
	if (!err) {
		registered_cb->state_changed(fp_adv_mode != APP_FP_ADV_MODE_OFF);

		LOG_INF("Fast Pair: advertising in the %s mode",
			fp_adv_mode_description[fp_adv_mode]);
	} else {
		registered_cb->state_changed(false);
...
static void connected(struct bt_conn *conn, uint8_t err)
{
	...
	if (err) {
		...
		//add by andrew
			if(0 == current_network_id_get())
			{//LOG_INF("#####connected_google_reboot######");
				//app_network_selector_set(APP_NETWORK_SELECTOR_GOOGLE);
				//factory_reset_reboot();
				//fmna_adv_stop();
				//execute_once_google =true;
				//k_msleep(200);
...
int app_fp_adv_init(const struct app_fp_adv_cb *cb)
{
	int err;
	int trigger_cnt;

	__ASSERT_NO_MSG(cb);
	__ASSERT_NO_MSG(cb->state_changed);
